-- Final fix for all student tables - simplified and error-free
-- This migration creates missing tables, adds missing columns, fixes foreign keys, and adds sample data

-- 1. Create student_progress table if it doesn't exist
...
-- 6. Add missing columns to existing tables (safe approach)
-- Add columns to student_progress if they don't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_progress' AND column_name = 'category') THEN
        ALTER TABLE student_progress ADD COLUMN category VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_progress' AND column_name = 'progress_value') THEN
        ALTER TABLE student_progress ADD COLUMN progress_value INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_progress' AND column_name = 'max_value') THEN
        ALTER TABLE student_progress ADD COLUMN max_value INTEGER DEFAULT 100;
    END IF;
END $$;

-- Add columns to student_activities if they don't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_activities' AND column_name = 'completed_at') THEN
        ALTER TABLE student_activities ADD COLUMN completed_at TIMESTAMP WITH TIME ZONE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_activities' AND column_name = 'points') THEN
        ALTER TABLE student_activities ADD COLUMN points INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_activities' AND column_name = 'activity_type') THEN
        ALTER TABLE student_activities ADD COLUMN activity_type VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_activities' AND column_name = 'title') THEN
        ALTER TABLE student_activities ADD COLUMN title VARCHAR(255);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_activities' AND column_name = 'description') THEN
        ALTER TABLE student_activities ADD COLUMN description TEXT;
    END IF;
END $$;

-- Add columns to student_goals if they don't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'target_value') THEN
        ALTER TABLE student_goals ADD COLUMN target_value INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'current_value') THEN
        ALTER TABLE student_goals ADD COLUMN current_value INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'category') THEN
        ALTER TABLE student_goals ADD COLUMN category VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'deadline') THEN
        ALTER TABLE student_goals ADD COLUMN deadline DATE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'completed') THEN
        ALTER TABLE student_goals ADD COLUMN completed BOOLEAN DEFAULT FALSE;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'title') THEN
        ALTER TABLE student_goals ADD COLUMN title VARCHAR(255);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_goals' AND column_name = 'description') THEN
        ALTER TABLE student_goals ADD COLUMN description TEXT;
    END IF;
END $$;

-- Add columns to student_milestones if they don't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_milestones' AND column_name = 'title') THEN
        ALTER TABLE student_milestones ADD COLUMN title VARCHAR(255);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_milestones' AND column_name = 'description') THEN
        ALTER TABLE student_milestones ADD COLUMN description TEXT;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_milestones' AND column_name = 'category') THEN
        ALTER TABLE student_milestones ADD COLUMN category VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_milestones' AND column_name = 'points') THEN
        ALTER TABLE student_milestones ADD COLUMN points INTEGER DEFAULT 0;
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_milestones' AND column_name = 'milestone_type') THEN
        ALTER TABLE student_milestones ADD COLUMN milestone_type VARCHAR(50);
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_milestones' AND column_name = 'achieved_at') THEN
        ALTER TABLE student_milestones ADD COLUMN achieved_at TIMESTAMP WITH TIME ZONE;
    END IF;
END $$;

-- Add columns to student_connections if they don't exist
DO $$
BEGIN
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_connections' AND column_name = 'connection_type') THEN
        ALTER TABLE student_connections ADD COLUMN connection_type VARCHAR(50) DEFAULT 'peer';
    END IF;
    IF NOT EXISTS (SELECT 1 FROM information_schema.columns WHERE table_name = 'student_connections' AND column_name = 'status') THEN
...
INSERT INTO student_milestones (student_id, title, description, category, points, milestone_type) VALUES
  ('584dd99a-6a33-4a57-9390-56f046e49231', 'First Course Completed', 'Completed your first online course', 'learning', 50, 'achievement'),
  ('584dd99a-6a33-4a57-9390-56f046e49231', 'Certificate Achiever', 'Earned your first professional certificate', 'certification', 100, 'achievement'),
  ('584dd99a-6a33-4a57-9390-56f046e49231', 'Project Builder', 'Created your first coding project', 'projects', 75, 'achievement');
