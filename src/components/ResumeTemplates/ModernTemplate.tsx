...
interface ResumeData {
  ...
  email: string;
  phone: string;
  overview?: string; // ✅ Added overview
  education: Education[];
...
interface ResumeProps {
  ...
  sectionTextSizes?: {
    header?: number;
    overview?: number; // ✅ Added overview font size
    education?: number;
    experience?: number;
    projects?: number;
    skills?: number;
...
const ModernTemplate = forwardRef<HTMLDivElement, ResumeProps>(
  ({ data, sectionTextSizes = {} }, ref) => {
    return (
      ...
      >
        {/* Header */}
        <header className='border-b border-slate-300 pb-3 mb-4 justify-center text-center'>
          <h1
        ...
        <section className='mb-4'>
          ...
          {data.projects.map((proj, idx) => (
            <div key={idx} className='mb-2 text-indigo-700'>
              <p
              ...
              {proj.details && proj.details.length > 0 && (
                <ul
                  ...
                  className='list-disc list-inside ml-4 text-indigo-700 mt-1'
                >
