    ...
    <Card>
      <CardHeader className='flex flex-row items-center justify-between space-y-0 pb-6'>
        ...
        <div className='flex items-center gap-2'>
          <Button size='sm' onClick={() => setIsProfileOpen(true)}>
            PROFILE
          </Button>
            ...
            <div className='flex items-center justify-between'>
              ...
              <Button
                ...
                onClick={() => {
                  ...
                  const links = [
                    ...(extras.linkedin
                      ? [{ label: 'LinkedIn', url: normalize(extras.linkedin) }]
                      : []),
                    ...(extras.github
                      ? [{ label: 'GitHub', url: normalize(extras.github) }]
                      : []),
                    ...(extras.twitter
                      ? [{ label: 'Twitter', url: normalize(extras.twitter) }]
                      : []),
                    ...(extras.website
                      ? [{ label: 'Website', url: normalize(extras.website) }]
                      : []),
                    ...customLinks.map(l => ({
                      label: l.label,
                      url: normalize(l.url),
                  ...
                  const linksHtml = links.length
                    ? `<ul>${links.map(l => `<li><a href="${l.url}" target="_blank" rel="noreferrer noopener">${l.label}: ${l.url}</a></li>`).join('')}</ul>`
                    : '<p class="muted">No links provided</p>';
                  const certsHtml = certificates.length
                    ? `<table><thead><tr><th>ID</th><th>Status</th></tr></thead><tbody>${certificates.map(c => `<tr><td>${c.id}</td><td>${c.status}</td></tr>`).join('')}</tbody></table>`
                    : '<p class="muted">No certificates yet</p>';
                  doc.document
                    .write(`<!doctype html><html><head><meta charset="utf-8"/><title>Resume - ${profile.full_name}</title><style>${styles}</style></head><body>
              ...
              <div className='grid grid-cols-1 md:grid-cols-2 gap-3'>
                <div className='space-y-1'>
                  ...
                  <div className='flex gap-2'>
                    <div className='inline-flex items-center px-2 border rounded-md text-sm text-muted-foreground'>
