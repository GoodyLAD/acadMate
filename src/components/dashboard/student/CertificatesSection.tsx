...
const CertificatesSection: React.FC<CertificatesSectionProps> = ({
  ...
  const academicCertificates = filteredCertificates.filter(
    cert => cert.category === 'academic'
  );
  const coCurricularCertificates = filteredCertificates.filter(
    cert => cert.category === 'co_curricular'
  );
      ...
      <CardContent className='p-4'>
        ...
        {(certificate.status === 'approved' ||
          ...
          (certificate.remark || certificate.rejection_reason) && (
            <div
              className={`mt-3 p-2 border rounded text-xs ${certificate.status === 'rejected' ? 'bg-red-50 border-red-200' : 'bg-green-50 border-green-200'}`}
            >
              <strong
                className={
                  certificate.status === 'rejected'
